...
def test_config_manager_update_and_get(config_manager):
    config_manager.update_settings({"github_token": "test_token_123", "forgejo_token": ""})
...
def test_api_settings_get(client_with_mocked_config, monkeypatch):
    ...
    # Configure via API
    response = client.post("/api/settings", json={"github_token": "secret_gh", "forgejo_url": "http://f.com", "forgejo_token": "secret_fg"}, headers=headers)
    assert response.status_code == 200
