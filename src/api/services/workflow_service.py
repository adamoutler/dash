...
def get_log_filename(provider: str, owner: str, repo: str, workflow_id: Optional[str] = None, branch: Optional[str] = None) -> str:
...
class WorkflowService:
    ...
    async def get_all_statuses(self, repos: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        tasks = []
        for r in repos:
            provider_instance = self._get_provider_instance(r.get("provider", ""))
            if provider_instance:
                tasks.append(provider_instance.fetch_status(
                    owner=r.get("owner"),
                    repo=r.get("repo"),
                ...
                async def mock_error():
                    return {"provider": r.get("provider", "unknown"), "owner": r.get("owner"), "repo": r.get("repo"), "status": "error", "commit_message": "Unsupported provider"}
    ...
    async def get_single_status(self, provider: str, owner: str, repo: str, workflow_id: Optional[str] = None, branch: Optional[str] = None) -> Dict[str, Any]:
        provider_instance = self._get_provider_instance(provider)
        if provider_instance:
            return await provider_instance.fetch_status(owner, repo, workflow_id, branch)
        return {"provider": provider, "owner": owner, "repo": repo, "status": "error", "commit_message": "Unknown provider"}

    def format_status_yaml(self, res_obj: Dict[str, Any], provider: str, owner: str, repo_name: str) -> str:
    ...
    async def get_artifacts(self, provider: str, owner: str, repo: str, workflow_id: Optional[str] = None, branch: Optional[str] = None) -> Dict[str, Any]:
        provider_instance = self._get_provider_instance(provider)
        if provider_instance:
            return await provider_instance.fetch_artifacts(owner, repo, workflow_id, branch)
    ...
    async def get_workflows(self, provider: str, owner: str, repo: str, branch: Optional[str] = None) -> List[Dict[str, str]]:
        provider_instance = self._get_provider_instance(provider)
        if provider_instance:
