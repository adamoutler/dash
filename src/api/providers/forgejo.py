...
class ForgejoProvider(BaseProvider):
    ...
    async def fetch_status(self, owner: str, repo: str, workflow_id: Optional[str] = None, branch: Optional[str] = None) -> Dict[str, Any]:
        ...
        headers = {"Authorization": f"token {self.token}", "Accept": "application/json"} if self.token else {}
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                return {
                    ...
                    "url": f"{run.get('html_url', self.url + '/' + owner + '/' + repo + '/actions/runs/' + str(run.get('index_in_repo', run.get('id', ''))))}/jobs/0/attempt/1" if run else "#",
